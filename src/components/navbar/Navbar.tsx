...
const Navbar = () => {
  ...
  return (
    <div
      className={`fixed z-20 w-full bg-white shadow-lg transition-all duration-1000 top-0 ${
        scrolling
          ? "border-b-4 bg-cover bg-no-repeat  border-blue-700 py-2 "
          : ""
      } `}
    >
      <nav className="container mx-auto px-10 flex justify-between py-2 lg:py-0  items-center h-[50px]">
        ...
        >
          {routes.map((item, idx) => (
            <Link
              className="hover-button hover:text-orange-500"
              key={idx}
              href={item.route}
            >
              <button
                className={`hover:text-orange-500 rounded ${
                  scrolling ? "text-black" : ""
                }
          ${pathName === item.route ? "text-orange-500 " : ""} ${
        ...
        <div className="block lg:hidden ">
          ...
          <div
            className={`  ${
              openSidebar ? "fixed inset-0 bg-black bg-opacity-50 " : " "
            }`}
          ></div>
          ...
          {!openSidebar && (
            <IoMenuSharp
              ...
              className="text-3xl text-gray-600 relative z-10"
            />
          )}

          <div
            onClick={() => setOpenSideBar(!openSidebar)}
            className={`bg-white shadow-2xl fixed transition-all duration-1000 z-50 top-0   flex flex-col   py-2  w-[200px]  pt-10 ${
          ...
          >
            ...
            {routes?.map((item, idx) => (
              <Link href={item.route} key={idx}>
                {" "}
                <p
                  className={`py-2 hover:bg-blue-900 hover:text-orange-500 px-6 border-b   ${
                    pathName === item.route ? "text-orange-500 " : ""
                  } `}
                >
                  {item.navItem}
